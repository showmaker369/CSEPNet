...
class XLSXRecoder(object):
    def __init__(self, xlsx_path):
        self.dataset_list = ["SD-saliency-900"]
        self.dataset_num_list = [360]
        self.metric_list = ["Smeasure", "MAE", "maxEm", 'meanFm', "maxFm"]
                ...
                for cell in row:
                    if cell.value == dataset_name.upper():
                        for i in range(num_metrics):
                            ...
                            key = list(data[dataset_name].keys()) #metrics list
                            sheet.cell(
                                row=idx_insert_row,
                                # column=cell.column + i+1,
                                column=2+i,
                                # value=data[dataset_name][matric_name],
                                value=data[dataset_name][key[i]],
                            )
        wb.save(self.path)
